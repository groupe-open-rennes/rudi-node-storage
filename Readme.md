...
Access to media-data, described in detailled below, can be one of the following:
    ...
    - *POST* https://<server_domain>/media/post
    - *GET*  https://<server_domain>/media/<media-uuid>
    - *GET*  https://<server_domain>/media/check/<media-uuid>
    - *GET*  https://<server_domain>/media/download/<media-uuid>
    - *GET*  https://<server_domain>/media/downloadz/<media-uuid>
...
```shell
curl -u 'rudiprod:xxxxxxxxxxxxxxxx'  -H 'file_metadata:{"media_name":"mon_nom","media_id":"37df63aa-1aae-4279-be3b-b07076d36131","file_size":21660,"file_type":"application/json"}' --data-binary @mon_nom.json https://<server_domain>/media/post
```
...
```shell
curl -u 'rudiprod:xxxxxxxxxxxxxxxx'  -H 'file_metadata:{"media_type":"INDIRECT", "media_name":"mon_nom","media_id":"37df63aa-1aae-4279-be3b-b07076d36888","url":"https://<server_domain>/media/pos
```
...
- _GET_ https://<server*domain>/media/UUID [in header: \_media-access-method*: [optional] access mode ]
...
```json
{ "url": "https://<server_domain>/media/storage/55643808-dd0c-48d9-941e-c21736d5e4e5" }
```

3. To directly download the content of a media-data:

- _GET_ https://<server_domain>/media/download/UUID
- _GET_ https://<server_domain>/media/zdownload/UUID
...
- _GET_ https://<server*domain>/media/check/UUID It is a
  shortcut for the standard media get with the \_media-access-method*
...
- _Context_ schema: [schema/rudi-media-db-context.json](https://<server_domain>/media/schema/rudi-media-db-context.json)
  Defines the context data recorded with all events.
- _Meta_ schema: [schema/rudi-media-db-meta.json](https://<server_domain>/media/schema/rudi-media-db-meta.json)
  Defines the minimal metadata requided for posting media.
- _File_ schema: [schema/rudi-media-db-file.json](https://<server_domain>/media/schema/rudi-media-db-file.json)
  Defines the data recorded for each media file.
- _URL_ schema: [schema/rudi-media-db-file.json](https://<server_domain>/media/schema/rudi-media-db-url.json)
  Defines the data recorded for each media URL.
- _Event_ schema: [schema/rudi-media-db-event.json](https://<server_domain>/media/schema/rudi-media-db-event.json)
...
[https://<server_domain>/media/logs/).
